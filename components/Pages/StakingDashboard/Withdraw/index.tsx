...
import { useUserData, useUserWithdraw } from "@/hooks/user";
...
const WithdrawScreen = () => {
  ...
  const [availableWithdrawAmountMessage, setAvailableWithdrawAmountMessage] =
    useState("");
  const [withdrawType, setWithdrawType] = useState<string>("Staking APY");
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const requestedKey: keyof UserWithdraws = `totalRequested${requestType}${formData.coin}`;
      ...
      >
        ...
        >
            ...
            <div>
              ...
              {withdrawType === "Staking APY" ? (
                <div className="grid grid-cols-3 gap-4">
                  {data &&
                    data.stakeInfo &&
                    Object.keys(data.stakeInfo).map((key, index) => {
                      ...
                      return (
                        <Card key={index}>
                          <CardContent className="flex flex-col">
                            ...
                            >
                              <p className="text-center">{stake.coin}</p>
                              <p className="text-center">{stake.amount}</p>
          ...
          <div className="bg-gradient-to-br from-[#00ff88] to-[#ffbb00] p-px rounded-lg">
            <button className="px-6 w-full text-center py-2 bg-[#11C7FF] text-black rounded-lg text-sm md:text-lg min-w-[150px] uppercase font-sem transition-all duration-300 dark:bg-[#2DE995] shadow-lg hover:bg-[#0FB8EC] focus:outline-none focus:ring-2 focus:ring-[#11C7FF] focus:ring-opacity-50">
              <span>
                {requestingWithdraw
                  ? "Requesting Withdraw ..."
                  : "Request Withdraw"}
